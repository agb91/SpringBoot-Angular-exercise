...
Some overview:
The application has a server-side, that is the brain of the social network, and some clients, one client for every user. So if we want Alice, Bob and Charlie we bootstrap the server, and we bootstrap 3 clients, one for Alice, one for Bob, one for Charlie. 
...
I've installed java 1.8.0_171, I'm pretty sure that all java versions >= 1.8 will be fine
...
https://github.com/agb91/SpringBoot-Angular-exercise

Here there are three projects: 
server side is in the folder social-kata-server
client side java based is in the folder social-kata-java-client
client side angular based is in the folder  social-kata-angular-client

Let's begin bootstrapping the server:
open a linux or windowns terminal, enter in the folder of the server social-kata-server. 
Enter the following 2 commands:
mvn clean package
mvn spring-boot:run

Now we can run one or more client consoles (one for each user that we want to use)
...
enter in the folder social-kata-java-client
Enter the following 2 commands:
mvn clean package
mvn spring-boot:run
...
follow the indications that the client application gives to you (about inserting your name, the command that you can use etc)
...
Because of simplicity, this is a very small project, I don't need formatted logs and different log levels
...
Which development technique have you used?
The project follows the Test Driven Development technique, but in a pragmatic way: for example the test coverage is not complete, and considering that the project is quite small the use of mocks in testing is very limited
...
Why Angular?
Because it is quite simple to learn and I wanted do add a (very basic) GUI to the project
